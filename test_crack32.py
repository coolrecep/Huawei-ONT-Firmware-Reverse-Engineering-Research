...
# Wait, `aescrypt2` needs to be run using `var/aescrypt2` which we created earlier (with 777 perms).
chan.send("/var/aescrypt2 0 /tmp/dummy.txt /tmp/dummy.enc\n")
out = recv_until(chan, "# ", timeout=5)
print("aescrypt2 out:", out)

chan.send("hexdump -C /tmp/dummy.enc\n")
out = recv_until(chan, "# ", timeout=5)
print("hexdump out:", out)
