...
# we can use hexdump to check the first bytes of hw_ctree.xml
chan.send("hexdump -C /mnt/jffs2/hw_ctree.xml | head -10\n")
out = recv_until(chan, "# ", timeout=5)
print(out)

# also check if there is an alternative encryption tool
chan.send("ls -la /bin/*crypt* /sbin/*crypt* /usr/bin/*crypt*\n")
out = recv_until(chan, "# ", timeout=5)
print(out)
