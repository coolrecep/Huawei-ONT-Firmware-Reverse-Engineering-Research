...
# Let's try passing arguments without dots, just node paths
chan.send("cfgtool get deftree X_HW_WebUserInfo\n")
out = recv_until(chan, "# ", timeout=5)
print(out)

# We want to change admin (Instance 1) to UserLevel 0
chan.send("cfgtool set deftree X_HW_WebUserInfoInstance.1 UserLevel 0\n")
out = recv_until(chan, "# ", timeout=5)
print(out)
